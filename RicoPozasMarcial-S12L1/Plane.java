...
public class  Plane
{
    ...
    /**
     * tercerc constructor con parametros de la clase Plane
     * @param identifier, un char que identifica el avion
     */
    public Plane(char identifier)
    {
        ...
        setPilot(new Person());
        setFuel(r.nextInt(MAX_FUEL+1));
        setXPos(r.nextInt(MAX_X+1));
        setYPos(r.nextInt(MAX_Y+1));
        sr = MAX_SPEED_X-MIN_SPEED_X;
        setXSpeed(r.nextInt(sr+1)+ MIN_SPEED_X);
        setYSpeed(r.nextInt(sr+1)+MIN_SPEED_Y);
        setIdentifier(identifier);
    }
